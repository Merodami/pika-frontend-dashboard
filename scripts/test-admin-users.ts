...
async function testAdminUsers() {
  try {
    ...
    try {
      ...
      log(`Users retrieved: ${usersResponse.data.data?.length || usersResponse.data.length || 0}`, colors.blue)
      log(`Total users: ${usersResponse.data.total || usersResponse.data.length || 0}`, colors.blue)
      ...
      if (Array.isArray(users) && users.length > 0) {
        ...
        users.slice(0, 3).forEach((user: any, index: number) => {
          log(`  ${index + 1}. ${user.email} (${user.role}) - ${user.status}`, colors.blue)
        })
      }
    ...
    if (usersResponse.data.data && usersResponse.data.data.length > 0) {
      log('\nFirst 3 users:', colors.cyan)
      usersResponse.data.data.slice(0, 3).forEach((user: any, index: number) => {
        log(`  ${index + 1}. ${user.email} (${user.role}) - ${user.status}`, colors.blue)
      })
    }
    ...
    if (error.response) {
      ...
      if (error.response.status === 404) {
        ...
        log('Make sure the admin user exists and credentials are correct', colors.blue)
      }
